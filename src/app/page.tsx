...
export default function FleetTrackerApp() {
  ...
  useEffect(() => { 
    if (activeTail) {
      localStorage.setItem('aft_active_tail', activeTail);
    }
  }, [activeTail]);
  ...
  const handlePilotOnboardingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/aircraft/create', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (isInitialLoad) {
    return (
      <div className="flex flex-col items-center justify-center p-4 bg-slateGray h-[100dvh] w-full overflow-hidden text-white font-oswald text-2xl tracking-widest uppercase animate-pulse">
        Loading...
      </div>
    );
  }
      ...
      <div className="flex flex-col bg-slateGray min-h-[100dvh] w-full overflow-y-auto">
        ...
        <div className="flex-1 p-4 flex justify-center items-start pt-8 pb-20">
          <div className="bg-cream shadow-2xl rounded-sm w-full max-w-lg p-6 md:p-8 border-t-4 border-[#F08B46] animate-slide-up">
            ...
            <div className="text-center mb-8">
              <h2 className="font-oswald text-3xl font-bold uppercase tracking-widest text-navy mb-2">
                Set Up Your Aircraft
              </h2>
              <p className="text-sm text-gray-500 font-roboto">
                Please enter your aircraft details to initialize your flight log and maintenance tracking.
              </p>
            </div>
            
            <form onSubmit={handlePilotOnboardingSubmit} className="space-y-4">
              ...
              <div className="pt-4">
                <PrimaryButton disabled={isSubmitting}>
                  {isSubmitting ? "Creating..." : "Create Aircraft & Enter Portal"}
                </PrimaryButton>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleSaveSettings} className="space-y-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div>
                  ...
                  <input 
                    type="number" 
                    value={sysSettings.reminder_1} 
                    onChange={e => setSysSettings({...sysSettings, reminder_1: parseInt(e.target.value)})} 
                    className="w-full border rounded p-2 text-sm mt-1 focus:border-navy outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="number" 
                    value={sysSettings.reminder_2} 
                    onChange={e => setSysSettings({...sysSettings, reminder_2: parseInt(e.target.value)})} 
                    className="w-full border rounded p-2 text-sm mt-1 focus:border-navy outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="number" 
                    value={sysSettings.reminder_3} 
                    onChange={e => setSysSettings({...sysSettings, reminder_3: parseInt(e.target.value)})} 
                    className="w-full border rounded p-2 text-sm mt-1 focus:border-navy outline-none" 
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input 
                    type="number" 
                    value={sysSettings.sched_time} 
                    onChange={e => setSysSettings({...sysSettings, sched_time: parseInt(e.target.value)})} 
                    className="w-full border rounded p-2 text-sm mt-1 focus:border-navy outline-none" 
                  />
                </div>
                <div>
                  ...
                  <input 
                    type="number" 
                    value={sysSettings.sched_days} 
                    onChange={e => setSysSettings({...sysSettings, sched_days: parseInt(e.target.value)})} 
                    className="w-full border rounded p-2 text-sm mt-1 focus:border-navy outline-none" 
                  />
                </div>
              </div>
              ...
              <div className="pt-4">
                <PrimaryButton disabled={isSubmitting}>
                  {isSubmitting ? "Saving..." : "Save Globally"}
                </PrimaryButton>
              </div>
            </form>
          </div>
