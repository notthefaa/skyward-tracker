...
import { sendMessageStream, CHUCK_MODEL } from '@/lib/chuck/claude';
...
export async function DELETE(req: Request) {
  try {
    ...
    await supabaseAdmin.from('aft_chuck_messages').delete().eq('thread_id', thread.id);
    await supabaseAdmin.from('aft_chuck_threads').delete().eq('id', thread.id);
...
// POST — send a message to Chuck (streamed SSE response)
