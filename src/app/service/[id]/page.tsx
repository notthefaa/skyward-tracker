...
import { 
  Wrench, AlertTriangle, CheckCircle, Clock, Send, ChevronDown,
  MessageSquare, Calendar, Sparkles, X, Plus
} from "lucide-react";
...
export default function ServicePortal() {
  ...
  if (isLoading) {
    return <div className="min-h-screen bg-neutral-100 flex items-center justify-center text-navy font-oswald tracking-widest uppercase text-xl">Loading Service Portal...</div>;
  }

  if (!event || !aircraft) {
    return <div className="min-h-screen bg-neutral-100 flex items-center justify-center text-navy font-oswald tracking-widest uppercase text-xl">Service Event Not Found</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-100 flex flex-col items-center p-4 md:p-8">
      ...
      <div className="w-full max-w-2xl space-y-6 animate-slide-up">
            ...
            {showDateForm && (
              <div className="mt-4 p-4 bg-gray-50 rounded border border-gray-200 space-y-3 animate-fade-in">
                ...
                <div>
                  ...
                  <textarea value={commentText} onChange={e => setCommentText(e.target.value)} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-[#F08B46] outline-none min-h-[80px]" placeholder="Any notes about availability..." />
                </div>
                <button onClick={() => handleAction('propose_date', { proposedDate, message: commentText })} disabled={isSubmitting || !proposedDate} className="w-full bg-[#F08B46] text-white font-oswald font-bold uppercase tracking-widest py-3 rounded active:scale-95 transition-transform disabled:opacity-50">
                  ...
                  <div className="flex justify-between items-start">
                    ...
                    {event.status !== 'complete' && (
                      <select 
                        value={li.line_status}
                        onChange={e => handleLineStatusUpdate(li.id, e.target.value)}
                        className="text-[10px] font-bold uppercase border border-gray-300 rounded px-2 py-1 bg-white focus:border-[#CE3732] outline-none"
                      >
                        <option value="pending">Pending</option>
                        <option value="in_progress">In Progress</option>
                        <option value="complete">Complete</option>
        ...
        {/* MECHANIC: SUGGEST ADDITIONAL WORK */}
        {event.status !== 'complete' && (
          <div className="bg-white shadow-lg rounded-sm p-6 border-t-4 border-[#F08B46]">
            <div className="flex justify-between items-center mb-4">
              <h3 className="font-oswald text-lg font-bold uppercase tracking-widest text-navy flex items-center gap-2"><Plus size={18} className="text-[#F08B46]"/> Suggest Additional Work</h3>
            </div>
            ...
            {messages.length === 0 ? (
              ...
              messages.map((msg: any) => (
                <div key={msg.id} className={`p-3 rounded text-sm ${msg.sender === 'mechanic' ? 'bg-blue-50 border-l-4 border-[#3AB0FF]' : msg.sender === 'owner' ? 'bg-orange-50 border-l-4 border-[#F08B46]' : 'bg-gray-50 border-l-4 border-gray-300'}`}>
                  ...
                  <p className="text-navy">{msg.message}</p>
                </div>
              ))
            )}
          </div>
