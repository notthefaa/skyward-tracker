...
export default function SquawksTab({ 
  ...
  return (
    <>
          ...
          {resolvedSquawks.length === 0 ? (<p className="text-center text-sm text-gray-400 italic py-4">No archived history.</p>) : (
            resolvedSquawks.map(sq => (
              <div key={sq.id} className="p-4 border border-gray-300 bg-white rounded opacity-70 hover:opacity-100 transition-opacity">
                <div className="flex justify-between items-start mb-2">
                  <div><span className="text-[10px] font-bold uppercase tracking-widest px-2 py-1 rounded text-white bg-gray-500">RESOLVED</span></div>
                  <div className="flex gap-3 items-center">
      ...
      {/* MODALS OMITTED FOR BREVITY IF YOU DON'T NEED ME TO REPEAT THEM, 
          BUT SINCE YOU ASKED FOR COMPLETE CODE, I WILL INCLUDE THE REST BELOW! */}
              ...
              {isTurbine && status === 'open' && (
                <div className="border border-blue-200 rounded p-4 bg-blue-50/30">
                  <label className="flex items-center gap-2 text-sm font-bold text-navy mb-4 cursor-pointer">
                    {/* NEW: Changed label to "Defer Item" */}
                    <input type="checkbox" checked={isDeferred} onChange={e=>setIsDeferred(e.target.checked)} className="w-4 h-4 cursor-pointer" /> 
                    Defer Item
                  </label>
