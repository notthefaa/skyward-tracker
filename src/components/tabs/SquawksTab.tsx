...
export default function SquawksTab({ 
  ...
  const submitSquawk = async (e: React.FormEvent) => {
    ...
    const squawkData = {
      ...
      reporter_initials: userInitials, 
      location, 
      description, 
  ...
  const handleShareMx = (sq: any) => { 
    ...
    body += `Secure Link:\n`;
    body += `${window.location.origin}/squawk/${sq.id}\n\n`;
    body += `---\n`;
              ...
              <div key={sq.id} className={`p-4 border rounded ${sq.affects_airworthiness ? 'border-[#CE3732]/30 bg-[#CE3732]/10' : 'border-[#F08B46]/30 bg-[#F08B46]/10'}`}>
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-3 items-center">
                    ...
                    {role === 'admin' && (
                      <>
                        <button onClick={() => resolveSquawk(sq)} className="text-gray-500 hover:text-green-600 active:scale-95" title="Mark Resolved">
                          <CheckCircle size={16}/>
                        </button>
                        <button onClick={() => deleteSquawk(sq.id)} className="text-gray-500 hover:text-red-600 active:scale-95" title="Delete">
                          <Trash2 size={16}/>
                        </button>
                      </>
                    )}
            ...
            resolvedSquawks.map(sq => (
              <div key={sq.id} className="p-4 border border-gray-300 bg-white rounded opacity-70 hover:opacity-100 transition-opacity">
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex gap-3 items-center">
                    <button onClick={() => openForm(sq)} className="text-gray-500 hover:text-gray-700 active:scale-95" title="View/Edit">
                      <Edit2 size={16}/>
                    </button>
                    {role === 'admin' && (
                      <button onClick={() => deleteSquawk(sq.id)} className="text-gray-500 hover:text-red-600 active:scale-95" title="Delete">
              ...
              {squawks.map(sq => (
                <label key={sq.id} className="flex items-start gap-3 p-3 border border-gray-200 rounded cursor-pointer hover:bg-gray-50 transition-colors">
                  <input 
                    ...
                    className="mt-1 w-4 h-4 text-[#CE3732] border-gray-300 rounded focus:ring-[#CE3732]" 
                  />
            ...
            <form onSubmit={submitSquawk} className="space-y-4">
                  ...
                  {isDeferred && (
                    <div className="space-y-4 animate-fade-in">
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                        ...
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-navy">NEF #</label>
                          <input type="text" value={nef} onChange={e=>setNef(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1 focus:border-[#CE3732] outline-none" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-navy">MDL #</label>
                          <input type="text" value={mdl} onChange={e=>setMdl(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1 focus:border-[#CE3732] outline-none" />
                        </div>
                      </div>
                      
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-navy">Control #</label>
                          <input type="text" value={melControl} onChange={e=>setMelControl(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1 focus:border-[#CE3732] outline-none" />
                        </div>
                        <div>
                      ...
                      <div className="pt-2">
                        <label className="flex items-start gap-2 text-xs font-bold text-navy cursor-pointer">
                          <input type="checkbox" required checked={procCompleted} onChange={e=>setProcCompleted(e.target.checked)} className="mt-0.5 w-4 h-4 cursor-pointer" /> 
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-navy">Name (Full Name) *</label>
                          <input type="text" required value={fullName} onChange={e=>setFullName(e.target.value)} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-[#CE3732] outline-none" />
                        </div>
                        <div>
                          <label className="text-[10px] font-bold uppercase tracking-widest text-navy">Certificate Number *</label>
                          <input type="text" required value={certNum} onChange={e=>setCertNum(e.target.value)} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-[#CE3732] outline-none" />
                        </div>
                      </div>
                ...
                <div className="pt-2 pb-2">
                  <label className="flex items-start gap-2 text-xs font-bold text-navy cursor-pointer">
                    <input 
                      type="checkbox" 
                      checked={notifyMx} 
                      onChange={e=>setNotifyMx(e.target.checked)} 
                      className="mt-0.5 w-4 h-4 text-[#CE3732] border-gray-300 rounded focus:ring-[#CE3732] cursor-pointer" 
                    />
              ...
              <div className="pt-4">
                <PrimaryButton disabled={isSubmitting}>
                  {isSubmitting ? "Saving..." : "Save Squawk"}
                </PrimaryButton>
              </div>
