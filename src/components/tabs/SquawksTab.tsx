...
import { AlertTriangle, Plus, X, Upload, Mail, Edit2, ChevronLeft, ChevronRight } from "lucide-react";
...
export default function SquawksTab({ aircraft, session, onGroundedStatusChange }: { aircraft: any, session: any, onGroundedStatusChange: () => void }) {
  ...
  // --- IMAGE PREVIEWER (LIGHTBOX) STATE ---
  ...
  const uploadImages = async (): Promise<string[]> => {
    ...
    // Compression Settings: Max 1MB, Max 1920px width/height
  ...
  const submitSquawk = async (e: React.FormEvent) => {
    ...
    const squawkData = {
      ...
      affects_airworthiness: affectsAirworthiness, status, pictures: allPictures,
  ...
  // --- LIGHTBOX NAVIGATION ---
  const nextImage = (e: React.MouseEvent) => { e.stopPropagation(); setPreviewIndex(prev => prev === (previewImages?.length || 1) - 1 ? 0 : prev + 1); };
  const prevImage = (e: React.MouseEvent) => { e.stopPropagation(); setPreviewIndex(prev => prev === 0 ? (previewImages?.length || 1) - 1 : prev - 1); };
  ...
  return (
    <>
                ...
                {sq.pictures && sq.pictures.length > 0 && (
                  <div className="mt-3 flex gap-2 overflow-x-auto pb-2">
                    {sq.pictures.map((pic: string, i: number) => (
                      <button key={i} onClick={() => { setPreviewImages(sq.pictures); setPreviewIndex(i); }} className="active:scale-95 transition-transform" title="Click to enlarge">
                        {/* loading="lazy" fixes the massive initial data payload! */}
      ...
      {/* FULLSCREEN IMAGE PREVIEWER (LIGHTBOX) */}
      {previewImages && (
        <div className="fixed inset-0 z-[60] bg-black/95 flex items-center justify-center animate-fade-in" onClick={() => setPreviewImages(null)}>
          <button className="absolute top-4 right-4 text-gray-400 hover:text-white transition-colors z-50 p-2" onClick={() => setPreviewImages(null)}><X size={32}/></button>
          
          {previewImages.length > 1 && (
            <button onClick={prevImage} className="absolute left-4 text-gray-400 hover:text-white transition-colors z-50 p-2"><ChevronLeft size={48}/></button>
          )}
          
          <div className="max-w-full max-h-full p-4 flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <img src={previewImages[previewIndex]} className="max-h-[85vh] max-w-full object-contain rounded shadow-2xl animate-fade-in" />
          </div>

          {previewImages.length > 1 && (
            <button onClick={nextImage} className="absolute right-4 text-gray-400 hover:text-white transition-colors z-50 p-2"><ChevronRight size={48}/></button>
          )}
      ...
      {/* SQUAWK REPORT MODAL (Unchanged functionality, compressed formatting) */}
      {showModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
          <div className="bg-white rounded shadow-2xl w-full max-w-lg p-6 border-t-4 border-brandOrange max-h-[90vh] overflow-y-auto animate-slide-up">
            ...
            <div className="bg-gray-50 p-3 rounded border border-gray-200 mb-4 grid grid-cols-2 gap-2 text-xs"><div><span className="font-bold text-gray-500 uppercase">Date:</span> {new Date().toLocaleDateString()}</div><div><span className="font-bold text-gray-500 uppercase">Tail:</span> {aircraft.tail_number}</div><div><span className="font-bold text-gray-500 uppercase">Serial:</span> {aircraft.serial_number || 'N/A'}</div><div className="truncate"><span className="font-bold text-gray-500 uppercase">By:</span> {reporterEmail}</div></div>
            <form onSubmit={submitSquawk} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Status</label><select value={status} onChange={e=>setStatus(e.target.value as any)} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-brandOrange bg-white font-bold"><option value="open">Open</option><option value="resolved">Resolved</option></select></div>
                <div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Affects Airworthiness?</label><select value={affectsAirworthiness ? "yes" : "no"} onChange={e=>setAffectsAirworthiness(e.target.value === "yes")} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-red-500 bg-white font-bold"><option value="no">No (Monitor)</option><option value="yes">YES (GROUNDED)</option></select></div>
              </div>
                ...
                <div className="border border-blue-200 rounded p-4 bg-blue-50/30">
                  ...
                  {isDeferred && (
                    <div className="space-y-4 animate-fade-in">
                      <div className="grid grid-cols-2 gap-3"><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">MEL #</label><input type="text" value={mel} onChange={e=>setMel(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">CDL #</label><input type="text" value={cdl} onChange={e=>setCdl(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">NEF #</label><input type="text" value={nef} onChange={e=>setNef(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">MDL #</label><input type="text" value={mdl} onChange={e=>setMdl(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div></div>
                      <div className="grid grid-cols-2 gap-3"><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Control #</label><input type="text" value={melControl} onChange={e=>setMelControl(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Category</label><select value={category} onChange={e=>setCategory(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1 bg-white"><option value="">Select...</option><option value="A">A</option><option value="B">B</option><option value="C">C</option><option value="D">D</option><option value="NA">N/A</option></select></div></div>
                      ...
                      <div className="pt-4 border-t border-gray-200"><label className="text-[10px] font-bold uppercase tracking-widest text-navy block mb-2">Signature <span className="text-red-500">*</span></label><div className="border border-gray-300 rounded bg-white"><SignatureCanvas ref={sigCanvas} penColor="black" canvasProps={{ className: 'w-full h-32 rounded' }} /></div><button type="button" onClick={() => sigCanvas.current?.clear()} className="text-[10px] font-bold uppercase text-gray-500 mt-1 hover:text-red-500">Clear Signature</button></div>
                      <div className="grid grid-cols-2 gap-4"><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Full Name <span className="text-red-500">*</span></label><input type="text" required value={fullName} onChange={e=>setFullName(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div><div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Certificate # <span className="text-red-500">*</span></label><input type="text" required value={certNum} onChange={e=>setCertNum(e.target.value)} className="w-full border border-gray-300 rounded p-2 text-sm mt-1" /></div></div>
                    </div>
                  )}
                </div>
              )}
