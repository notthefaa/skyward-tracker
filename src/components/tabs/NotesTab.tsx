...
export default function NotesTab({ aircraft, session, onNotesRead }: { aircraft: any, session: any, onNotesRead: () => void }) {
  ...
  useEffect(() => { if (aircraft) fetchNotes(); }, [aircraft?.id]);
  ...
  const openForm = (note: any = null) => {
    if (note) { setEditingId(note.id); setContent(note.content || ""); setExistingImages(note.pictures ||[]); } 
    else { setEditingId(null); setContent(""); setExistingImages([]); }
    setSelectedImages([]); setShowModal(true);
  };
  ...
  const submitNote = async (e: React.FormEvent) => {
    ...
    const noteData: any = { aircraft_id: aircraft.id, content, pictures: allPictures };

    if (editingId) {
      noteData.edited_at = new Date().toISOString();
      await supabase.from('aft_notes').update(noteData).eq('id', editingId);
    } else {
      noteData.author_id = session.user.id; noteData.author_email = session.user.email;
      await supabase.from('aft_notes').insert(noteData);
    }
    await fetchNotes(); setShowModal(false); setIsSubmitting(false);
  };
