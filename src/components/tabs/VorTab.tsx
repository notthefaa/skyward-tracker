    ...
    <>
      <div className="mb-2">
        <button onClick={openForm} className="flex items-center gap-2 bg-navy text-white font-oswald text-sm font-bold uppercase tracking-widest px-4 py-2.5 rounded-lg shadow active:scale-95 transition-transform w-full justify-center">
          <Plus size={16} /> Log VOR Check
        </button>
      </div>

      <div className="bg-cream rounded-lg shadow border-t-4 border-[#F08B46] p-4 md:p-6">
        {/* Due status banner */}
        <div className={`rounded-lg border-2 ${due.bgColor} px-4 py-3 mb-4 flex items-center justify-between`}>
          <div>
        ...
        <div className="flex justify-between items-start mb-3">
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400">FAR 91.171</span>
            <h2 className="font-oswald text-lg font-bold uppercase tracking-widest text-navy">VOR Check Log</h2>
          </div>
        </div>
            ...
            <tbody className="text-xs font-roboto text-navy">
              ...
              {vorChecks.map((c, i) => (
                <tr key={c.id} className="border-b border-gray-200 hover:bg-blue-50/50 transition-colors">
                  ...
                  <td className="py-2.5 pr-3 whitespace-nowrap text-[10px] font-bold uppercase tracking-wider">{c.check_type}</td>
                  <td className="py-2.5 pr-3 whitespace-nowrap">{c.station}</td>
                  <td className="py-2.5 pr-3 whitespace-nowrap font-bold">{c.bearing_error > 0 ? '+' : ''}{c.bearing_error}°</td>
