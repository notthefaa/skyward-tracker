    ...
    <div className="flex flex-col gap-4 animate-fade-in relative">
      ...
      <div className="bg-white shadow-lg rounded-sm overflow-hidden">
        <div className="relative h-40 md:h-56 bg-slateGray flex items-center justify-center">
          ...
          {role === 'admin' && (
            <button onClick={() => setShowDeleteModal(true)} className="absolute top-4 right-4 bg-[#CE3732] text-white p-2.5 rounded-full shadow-[0_4px_10px_rgba(0,0,0,0.5)] hover:bg-red-700 active:scale-95 transition-all z-10" title="Delete Aircraft">
              <Trash2 size={18} />
            </button>
          )}
      ...
      <div className="bg-white shadow-lg rounded-sm p-4 border-t-4 border-blue-500 flex flex-col">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-3xl font-roboto font-bold text-navy">{fuelGals.toFixed(1)} <span className="text-sm text-gray-400">Gal</span></p>
          </div>
          <div>
            <span className="text-[10px] font-bold uppercase tracking-widest text-gray-400 block mb-1">Weight</span>
            <p className="text-3xl font-roboto font-bold text-navy">{fuelLbs.toLocaleString()} <span className="text-sm text-gray-400">Lbs</span></p>
          </div>
        </div>
      </div>
            ...
            <div className="flex-1">
              ...
              {nextMx ? (
                <>
                  ...
                  <p className={`text-xs font-bold mt-0.5 ${mxTextColor}`}>{nextMx.dueText}</p>
                </>
              ) : (
