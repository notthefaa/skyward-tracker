 ...
 */
export function SecondaryButton({ children, onClick, disabled, type = "button" }: { children: ReactNode, onClick?: () => void, disabled?: boolean, type?: "button" | "submit" }) {
  return (
    <button
      type={type}
      onClick={onClick}
      disabled={disabled}
      className="w-full border border-gray-300 text-gray-600 font-oswald font-bold tracking-widest uppercase py-3 px-4 rounded hover:bg-gray-50 active:scale-95 transition-all duration-150 ease-out flex justify-center items-center gap-2 text-sm disabled:opacity-50 disabled:active:scale-100 disabled:cursor-not-allowed"
    >
      {children}
    </button>
  );
