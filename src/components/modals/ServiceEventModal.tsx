      ...
      <div className="mt-3 pt-3 border-t border-gray-200">
        ...
        <div className="flex gap-2 flex-wrap">
          {attachments.map((att: any, idx: number) => {
            ...
            if (isImg) {
              return (
                <button key={idx} onClick={() => setViewingAttachment(att.url)} className="w-16 h-16 rounded border-2 border-gray-200 overflow-hidden hover:border-[#3AB0FF] transition-colors active:scale-95">
                  <img src={att.url} alt={att.filename} className="w-full h-full object-cover" />
                </button>
              );
            }
            return (
              <a key={idx} href={att.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded hover:border-[#3AB0FF] transition-colors">
                <FileText size={14} className="text-gray-500 shrink-0" />
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-navy truncate max-w-[100px]">{att.filename}</p>
                  {att.size && <p className="text-[8px] text-gray-400">{att.size < 1024 * 1024 ? (att.size / 1024).toFixed(0) + ' KB' : (att.size / (1024 * 1024)).toFixed(1) + ' MB'}</p>}
                </div>
              </a>
            );
          })}
        ...
        <div className="bg-white rounded shadow-2xl w-full max-w-lg p-6 border-t-4 border-[#F08B46] max-h-full overflow-y-auto animate-slide-up" style={{ overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' }} onClick={e => e.stopPropagation()}>
          ...
          {view === 'list' && (
            <div className="space-y-4">
              <PrimaryButton onClick={openCreateFlow}><Calendar size={18} /> Schedule New Service</PrimaryButton>
          ...
          {view === 'create' && (
            <div className="space-y-6">
              ...
              <div><label className="text-[10px] font-bold uppercase tracking-widest text-navy">Preferred Service Date (Optional)</label><input type="date" value={proposedDate} onChange={e => setProposedDate(e.target.value)} className="w-full border border-gray-300 rounded p-3 text-sm mt-1 focus:border-[#F08B46] outline-none bg-white" /></div>
          ...
          {view === 'detail' && selectedEvent && (
            <div className="space-y-5">
              ...
              <div className="bg-gray-50 rounded p-4 border border-gray-200">
                <div className="flex justify-between items-center mb-3">
                  ...
                  {selectedEvent.access_token && selectedEvent.status !== 'complete' && (
                    <a href={`/service/${selectedEvent.access_token}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-[#3AB0FF] bg-blue-50 border border-blue-200 rounded px-3 py-1.5 hover:bg-blue-100 active:scale-95 transition-all"><ExternalLink size={12} /> Portal</a>
                  )}
                </div>
                    ...
                    {eventLineItems.map(li => (
                      <div key={li.id} className={`p-3 border rounded text-sm ${li.line_status === 'complete' ? 'bg-green-50 border-green-200' : li.line_status === 'in_progress' ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200'}`}>
                        <div className="flex justify-between items-start">
                          <div><span className="font-bold text-navy">{li.item_name}</span>{li.item_description && <p className="text-[10px] text-gray-500 mt-0.5">{li.item_description}</p>}</div>
              ...
              {/* Message Thread — WITH ATTACHMENT RENDERING */}
