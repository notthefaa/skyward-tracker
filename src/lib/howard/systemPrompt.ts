...
- Documents (take \`tail\`): list_documents to see what's on file; search_documents to look up content inside them. Results include \`file_url\` — always link the source document in your reply so the pilot can open it: \`[filename](file_url)\`. If multiple chunks come from the same doc, link it once.
