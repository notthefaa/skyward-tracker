...
export async function middleware(req: NextRequest) {
  ...
  // Touching getUser() triggers @supabase/ssr's refresh-if-needed
  // logic. The result is intentionally discarded — we don't enforce
  // auth here; downstream route handlers do. The point is just to
  // keep the cookie current so handlers don't see a stale token.
  await supabase.auth.getUser().catch(() => {
    // Network blip / GoTrue blip → leave cookie as-is, downstream
    // route handler will surface the 401 if needed.
  });
...
export const config = {
  matcher: [
     ...
     * - favicon.ico, manifest.webmanifest, sw.js, robots.txt, sitemap.xml
     * - public/* (anything in /public served at the root)
     * - cron/* (CRON_SECRET-gated, no user auth)
     */
    '/((?!_next/static|_next/image|favicon\\.ico|manifest\\.webmanifest|sw\\.js|robots\\.txt|sitemap\\.xml|api/cron/).*)',
  ],
};
